...
| `merch_admin` | `/admin/merch-v2` | **Phase 10:** Cockpit; Stammdaten **Farben und Grössen** (`GET`/POST unter `…/lookups`), Sortiment `/articles`; **Bulk-Deaktivierung** von Varianten pro Artikel (`POST …/articles/<id>/variants/bulk-deactivate-color`, `…/bulk-deactivate-size`); Lieferanten (`/suppliers`); Bild nach Drive (`MERCH_ARTICLE_IMAGE_DRIVE_FOLDER_ID`); Runden-Lifecycle; CSV; Lieferantenbeleg; Jahresstatistik (`/statistics`, Export); Bestellungen «Abgeholt»/«Bezahlt». Legacy-Forderungen: `admin` `/admin/merch/legacy-receivables`. Klassischer Merch-Admin (`/admin/merch`): nur Deep-Link. |
