...
| `admin` | `/admin` | Vereinsverwaltung: `GET /admin` und `GET /admin/` leiten zum Verein-Hub (`member.index`) um; Themen (Mitglieder, Merch-Cockpit, Legacy-Altbestand) sind dort verlinkt. Mitgliederliste **lesend** fuer alle aktiven Mitglieder (`verein_member_required`); Bearbeiten, sensible Daten, Security, Mail-Test und Mutationen nur gemaess den jeweiligen Routen (`admin_required` wo noetig). Klassischer Merch-Admin (`/admin/merch`) bleibt in der Codebasis fuer Deep-Links, ist aber nicht mehr in der Navigation. |
| `docs` | `/docs` | **Phase 03 + 09:** Vereinsdokumente im Google Shared Drive – Drive-Browser (Ordner-Tiles, Breadcrumb, Dateiliste), Detail, Upload, Rename, Move, Archive, Restore, Hard-Delete, Download. Sichtbar nur bei `DRIVE_FEATURE_ENABLED=true`. `DriveStorageService.admin_full_resync` existiert weiter fuer manuelle Notfaelle; es gibt keinen UI-Button mehr (Phase-9-Flow). Spec: `docs/capabilities/drive.md`. |
...
| `merch` | `/merch` | **Phase 10:** Merch v2 nur wenn `MERCH_V2_ENABLED=true`: `GET /` Übersicht; `GET /rounds/<id>` Runden-Shop (**Konfigurator**: ein Block je Artikel, Ausführung per Dropdown statt langer Variantenliste; Farb-/Grössen-Labels aus Lookups, sonst Attribut-JSON); `POST /rounds/<id>/cart`, `POST /rounds/<id>/confirm`, `POST /rounds/<id>/cancel-order`; `GET /merch/image/<article_id>` Bild-Proxy (Drive, Redis-Cache). Spec: `docs/capabilities/merch.md`. |
| `merch_admin` | `/admin/merch-v2` | **Phase 10:** Cockpit; Stammdaten **Farben** / **Grössen** (`GET`/`POST` unter `…/lookups/colors` bzw. `…/lookups/sizes`, Alias `…/lookups` → Farben), Sortiment `/articles`; **Bulk-Deaktivierung** von Varianten pro Artikel (`POST …/articles/<id>/variants/bulk-deactivate-color`, `…/bulk-deactivate-size`); Lieferanten (`/suppliers`); Bild nach Drive (`MERCH_ARTICLE_IMAGE_DRIVE_FOLDER_ID`); Runden-Lifecycle; CSV; Lieferantenbeleg; Jahresstatistik (`/statistics`, Export); Bestellungen «Abgeholt»/«Bezahlt». Legacy-Forderungen: `admin` `/admin/merch/legacy-receivables`. Klassischer Merch-Admin (`/admin/merch`): nur Deep-Link. |
...
- **`MerchSupplier`**, **`MerchArticle`**, **`MerchColor`**, **`MerchSize`**, **`MerchVariant`**, **`MerchRound`**, **`MerchRoundItem`**, **`MerchOrder`**, **`MerchOrderItem`** – Merch v2 (**Phase 10**, Modul `backend/models/merch_v2.py`; Migrationen `a9c81e2d4f03` + **`c4e8a9012b71`** fuer Farb-/Groessen-Lookups und `variant_key`; Spec `docs/capabilities/merch.md`, Zielbild `docs/capabilities/merch-article-variant-target-state.md`).
...
| `MerchSortimentService` | **Phase 10:** Kombinationen aus `variant_schema`, Lookups **Farbe/Grösse** (`merch_colors`, `merch_sizes`), **variant_key** (`backend/utils/merch_variant_key.py`), Listenpreis-Auflösung, Parsing/Caps für Cockpit-Textarea, Abgleich Variantenzeilen (`docs/capabilities/merch.md`, Zielbild `merch-article-variant-target-state.md`). |
| `MerchLookupService` / `MerchVariantBulkService` | **Phase 10:** Cockpit-Stammdaten Farbe/Grösse (Listen, anlegen, Label umbenennen) und Massen-Deaktivierung von Varianten eines Artikels nach `color_id`/`size_id` (`backend/services/merch_lookup_service.py`). |
