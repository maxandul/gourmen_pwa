...
| `merch_admin` | `/admin/merch-v2` | **Phase 10:** Cockpit, Stammdaten Lieferanten (`/suppliers`), **Sortiment Artikel** (`/articles`, Bild nach Drive mit `MERCH_ARTICLE_IMAGE_DRIVE_FOLDER_ID`), Runden-Lifecycle, CSV-Aggregat, Lieferantenbeleg-Upload, Jahresstatistik (`/statistics`, `/statistics/export.csv`), Bestellungen «Abgeholt»/«Bezahlt». Legacy-Forderungen read-only: `admin` `/admin/merch/legacy-receivables`. Klassischer Merch-Admin (`/admin/merch`, Legacy-Tabellen): nur noch per direkter URL, nicht in der App-Navigation. |
...
| `MerchSortimentService` | **Phase 10:** Kombinationen aus `variant_schema`, Listenpreis-Aufloesung Artikel/Variante, Parsing/Caps fuer Cockpit-Textarea, Abgleich Variantenzeilen (`docs/capabilities/merch.md`). |
