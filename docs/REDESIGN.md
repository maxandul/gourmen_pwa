...
| Bereich                                                                  | Inhalt                                                                                                                                                                                                                                             |
| ------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Dashboard**                                                            | Persönlicher **Überblick** nach **Nutzerintention**: **Zu erledigen** (**Datenbereinigung** **`events.cleanup`** — **§6.3**: Fenster **heute+30 Tage** nur RSVP, **vergangene** ab Stichtag inkl. Bewertung, **jüngstes zuerst**; Kachel-Untertitel **„fehlende Angabe(n)“**); separate Kachel **Bewertung ausstehend** → Event-Detail **`tab=ratings`** wenn **`rating_prompt`**. **Zur Info** (**Nächstes Event** → Detail **ohne** RSVP auf der Kachel; Zu-/Absage wie bisher **Event-Detail** / **Events-Liste**); **Dein letzter Anteil**; **GGL** (Untertitel **ohne** Teilnahme-Zähler); **Merch** (nicht geliefert). **Erkunden** nur **Merch-Shop** (Untertitel **„Shop und Bestellungen“**) und **Statistiken** (**„KPIs und Charts“**) — **Events** über **Bottom-Nav**. Optional **Push-Banner**. Vollständige Copy-Tabelle **§6.3**.                                                                                                                             |
...
| Dashboard (Intent-Layout) | **`.dashboard-intent`**, **`.dashboard-intent__heading`**, **`.dashboard-intent__grid`**, **`.dashboard-intent-tile`** (+ **`__icon`**, **`__body`**, **`__title`**, **`__meta`**, **`__chev`**), Modifier **`dashboard-intent-tile--static`**. **`dashboard-intent__stack`** existiert in **`components.css`**, wird auf dem **Dashboard** nicht mehr genutzt (alle drei Sektionen: Kacheln in **`__grid`** — **§6.3**). **Legacy / ungenutzt auf Dashboard:** **`.dashboard-next-event*`** (CSS vorhanden), **`a.card--dash-tile__hit`**, **`.card--dash-tile__actions`**, **`.card--dash-tile`**, **`.dashboard-card-link`**, **`.dashboard-hygiene-rows`**, **`.dashboard-row-link`**, **`dashboard-row-link--block-start`**. | Drei Sektionen **Zu erledigen** / **Zur Info** / **Erkunden**; knappe Kacheln. CSS: **`components.css`** „DASHBOARD“. |
...
**Layout (Stand 2026-04-06):** Jede der drei Sektionen **Zu erledigen**, **Zur Info**, **Erkunden** umschließt ihre Kacheln mit **`dashboard-intent__grid`** (mobil 1 Spalte, ab **480px** 2 Spalten). Es gibt **kein** **`dashboard-intent__stack`** mehr im Dashboard-Template — einheitliche Raster-Anordnung.

**Implementierte Pfade (Stand 2026-04-05, Layout ergänzt 2026-04-06):**

| Bereich | Datei / Ort |
| --------|-------------|
| Template | **`templates/dashboard/index.html`** — Push-Banner; **Zu erledigen** (Cleanup mit **`brush-cleaning`**, Untertitel **„1 fehlende Angabe“** / **„n fehlende Angaben“**; Bewertung); **Zur Info** wie §4.1 (**GGL**-Kachel **ohne** „Teilnahme x/x“ im Untertitel); **Erkunden:** **Merch-Shop** Untertitel **„Shop und Bestellungen“**, **Statistiken** **„KPIs und Charts“**. Jinja-Makro **`dashboard_intent_tile`**. Sämtliche Kachel-Blöcke: **`dashboard-intent__grid`**. |
...
**Datenbereinigung (`events.cleanup`, `RetroCleanupService`):** **Kandidaten** = Vereinigung aus **Upcoming-Fenster** (**`datum`** zwischen **Tagesbeginn UTC** und **Ende des Tages heute+30**) und **Retro-Past** (**`datum ≤ utcnow − CUTOFF_DAYS`**), je Mitglied gefiltert nach **Beitritt**. **Nächstes angezeigtes Event:** erstes **Offenes** in absteigender **`datum`**-Reihenfolge. **`allows_cleanup_rsvp`:** Upcoming-Fenster **oder** Retro-Past. **Bewertungskarte** nur wenn **nicht** Upcoming-Fenster.

**Seite `templates/events/cleanup.html` (Copy, Stand 2026-04-06):** **`{% block title %}`** „Bereinigung“; **`h1`:** Lucide **`brush-cleaning`** + „Bereinigung“; **`.page-back`** → **`dashboard.index`**. **`.events-cleanup-intro`:** ein Satz aus **`progress.pending`** — „Du hast keine fehlenden Angaben mehr.“ (0) / „Du hast 1 fehlende Angabe.“ (1) / „Du hast *n* fehlende Angaben.“ (*n* ≥ 2). Keine ausführliche Fenster-/Stichtag-Einleitung mehr im Template (Logik **§4.1** / **`RetroCleanupService`**).
...
| **Verein-Hub** | `templates/member/index.html`, `backend/routes/member.py` → **`index`** | **`settings-nav`**: Sektion **Verein** (eine Zeile **Merch** → `member.merch`, Beschreibung **„Shop und Bestellungen“**); Sektion **Verwaltung** nur **`is_admin()`**: **Mitglieder** (`admin.members`), **Merch-Verwaltung** (`admin.merch`). **`h1` „Verein“**. Route **`member.index`** ohne zusätzlichen Template-Context. |
| **Dashboard-Copy** | `templates/dashboard/index.html` | Kachel **Datenbereinigung:** „1 fehlende Angabe“ / „n fehlende Angaben“; **GGL**-Meta **ohne** Teilnahme-Zähler; **Erkunden:** Merch-Shop **„Shop und Bestellungen“**, Statistiken **„KPIs und Charts“** (zur Pflege siehe **§6.3**). |
...
**Technische Anker:** **`backend/routes/member.py`** — **`member.index`** rendert **`member/index.html`** ohne Extra-Context. **`retro_cleanup`**, **`dashboard.py`**: unverändert für Phase 7 relevant. **CSS-Fingerprint:** nach Änderungen an **`components.css`** immer **`python scripts/fingerprint_assets.py`**. Branch **`redesign`**.
...
**Phase 5:** Dashboard-Intent + Cleanup-Logik **umgesetzt**; **`templates/dashboard/index.html`** in **§13** Status **pending** bis PO-Freigabe oder explizit **`done`**. Layout: **§6.3** (**`dashboard-intent__grid`** für alle Sektionen, Stand 2026-04-06). **`events/cleanup.html`:** **Phase 7** erledigt (**§8.3.1**); Copy **§6.3** / **§12** (2026-04-06).
