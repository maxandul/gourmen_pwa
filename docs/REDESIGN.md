...
**CSS:** `static/css/v2/components.css` (Abschnitte „TOOL SURFACE“, „DISCLOSURE“, „CONTEXT ACTIONS“ [Legacy], „TOOL-STRIP“ / **`.tool-strip__actions`**, „SETTINGS NAV“, „USER MENU“, „EMPTY STATE“, „BILLBRO WORKFLOW“, „EVENT RATINGS“, „DASHBOARD“, Bereinigung **`.cleanup-step-nav*`** / **`.events-cleanup-*`** / **`.cleanup-undo-form`**, Modifier **`billbro-guess-ranking-table`** / **`events-ratings-others-table`**).
...
| **Dashboard-Copy** | `templates/dashboard/index.html` | **Zu erledigen:** Kacheln **Bereinigung** + **Zu-/Absage** (Copy **§6.3**); **GGL**-Meta **ohne** Teilnahme-Zähler; **Erkunden:** Merch-Shop **„Shop und Bestellungen“**, Statistiken **„KPIs und Charts“**. |
...
**Technische Anker:** **`backend/routes/member.py`** — **`member.index`** rendert **`member/index.html`** ohne Extra-Context. **Dashboard / Bereinigung:** **`backend/routes/dashboard.py`**, **`backend/services/retro_cleanup.py`**, **`backend/routes/events.py`** (`cleanup`, `cleanup_rsvp`, `cleanup_undo_rsvp`), **`backend/routes/ratings.py`** (Undo-Clear), **`app.py`** (`inject_retro_cleanup`) — Details **§6.3**. **CSS-Fingerprint:** nach Änderungen an **`components.css`** immer **`python scripts/fingerprint_assets.py`**. Branch **`redesign`**.
