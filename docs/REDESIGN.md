...
| Tabs                   | `.tabs`, `.tabs__nav`, `.tabs__tab`, `.tabs__panel`; optional **`.tabs--panel`**: **Nav** **transparent**, **`gap`** zwischen Tabs; **inaktiv** Light **`--color-bg-muted`**, Dark **`--chrome-inactive-pill-bg`** (Selektor **`:not(.tabs__tab--active)`**, damit der aktive Tab nicht überschrieben wird); **aktiv** + **`tabs__content`** = **`--tabs-panel-body-bg`** (`**--color-surface**`); Tabs **oben abgerundet**, **ohne** sichtbaren Einzel-Rand; untere Panel-Kante mit dezenter Linie/Schatten — Pilot **`templates/events/index.html`** |
...
| Bereichs-Tabs mit Listen/Karten darunter        | Optional **`.tabs.tabs--panel`**: **§5.1** — transparente Nav, **Gap**, inaktive Tabs getönt (Light/Dark wie dort), aktiver Tab = Fläche des Panels; **`.tabs__content`** mit Innenabstand — Pilot **Events-Index** (`components.css`) |
