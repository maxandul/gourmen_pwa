...
**Nächster Phase-7-Block:** Paket **7f** (Abschluss **§16**, **`base.html`** / Dashboard **§13**, Registry), siehe **§8.3.1**.
...
- **2026-04-06 (Paket 7e — Auth):** Alle **`templates/auth/*.html`** auf V2: **`use_v2_design`**, gemeinsames Macro **`partials/_lucide_icon.html`** (Sprite **`url_for`**), keine Inline-Pfad-SVGs; Formular-Hinweise **`form-field__help`**; **`verify_2fa`** / **`disable_2fa`** / **`backup_codes`**: Alerts mit Lucide-**`alert__icon`**; **`show_reset_link`:** **`{% block title %}`** ergänzt. **`backup_codes`:** Druck-/Kopier-/QR-Aktionen in **`card__footer`**. **Nächster Schritt:** **Paket 7f** (**§8.3.1**).
...
Unter **„übrige `templates/`**“ fällt der **Abschluss** nach **§8.3.1** (**Paket 7f** — **`base.html`**, Dashboard-**`done`** nach PO, **§16**); Auth-Templates (**7e**) sind **erledigt** (**§8.3.1**).
