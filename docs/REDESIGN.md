...
1b. **Zu-BillBro-Kachel (nur heute):** Veröffentlichtes Event mit **`datum`** im **Kalendertag UTC heute** (`day_start` … `day_end`), nur nach **`beitritt`** wie oben; **Organisator** **oder** **`Participation.teilnahme`**; bei mehreren Events am selben Tag das **früheste** **`datum`**. Link: **Event-Detail** **`tab=billbro`**. Icon **`receipt`** wie Tab BillBro. **Nicht** angezeigt ohne Zusage (außer Organisator).
...
**Ursprüngliche Entscheidung** (2026-04-05, PO/User — ältere **§12**-Zeilen): Nacharbeit zu Events über Dashboard; **`rating_prompt`** / Bewertungs-Zeile — **superseded** durch **§6.3** (2026-04-06): **Zu erledigen** mit **Bereinigung** + **Zu-/Absage** (und seit 2026-04-06 optional **Zu BillBro** am Eventtag), keine separate Bewertungs-Kachel.

**Aktueller Soll-Zustand** (verbindlich **§6.3**):

1. **Zu erledigen:** (a) **Zu BillBro** → **`events.detail`** **`tab=billbro`** am **Eventtag** (UTC), wenn **`today_billbro_event`**; (b) **Bereinigung** → **`events.cleanup`** bei **`retro_cleanup_progress.pending > 0`**; (c) **Zu-/Absage** → **`events.detail`** für **`rsvp_prompt_event`** (kommende 30 Tage, frühestes offenes RSVP). Bewertungen vergangener Events nur über **Bereinigung**.
...
| **Dashboard-Copy** | `templates/dashboard/index.html` | **Zu erledigen:** Reihenfolge **Zu BillBro** (nur Eventtag, **§6.3** 1b) + **Bereinigung** + **Zu-/Absage** (Copy **§6.3**); **GGL**-Meta **ohne** Teilnahme-Zähler; **Erkunden:** Merch-Kachel Titel **„Merch“**, Icon **`shirt`**, Meta **„Shop und Bestellungen“**; Statistiken **„KPIs und Charts“**. |
...
**Stand:** Pakete **7a–7f** sind in **§8.3.1** **erledigt**. Technische Abschlussarbeiten (V1-Entfernung, Backlog **§16.2**, GGL-Redirect) sind umgesetzt; **`templates/dashboard/index.html`** bleibt in **§13** auf **pending**, bis der **PO** das Intent-Dashboard explizit freigibt (**§16.1** Punkt 1 ggf. danach vollständig).

**Branch:** ausschließlich **`redesign`** committen; **kein** Push auf **`master`** ohne ausdrückliche User-Vorgabe (**§15**).

**Bei Merge-Vorbereitung:** **§16** Einleitung und **§16.1** mit dem **User** gegenprüfen (insbesondere Dashboard-**§13** und verbleibende **§16.2**-Nachzieher, falls später ergänzt).
...
| 7                  | erledigt | Paket **7f** abgeschlossen (**§8.3.1**); **`§13`** **`dashboard/index.html`** weiterhin **pending** bis PO-Freigabe; Gesamt-**§16.1**-GO mit **User** klären. |


### NAECHSTER SCHRITT

**Redesign-Umsetzung (Phasen 0–7):** abgeschlossen. **Offen für Produkt/PO:** **`templates/dashboard/index.html`** in **§13** auf **`done`** setzen, wenn das Intent-Dashboard visuell freigegeben wird; danach **§16.1** Punkt 1 für die Template-Übersicht vollständig erfüllbar.
...
- **2026-04-06 (Dashboard):** **Zu erledigen** — neue Kachel **„Zu BillBro“** zuoberst (Lucide **`receipt`**), nur am **Eventtag** (UTC), Link **`tab=billbro`**; **`RetroCleanupService.get_today_billbro_prompt_event`**. Doku **§4.1**, **§6.3**, **§8.1**, **§12**.
...
| 2026-04-06 | 5 | **Dashboard „Zu BillBro“:** Kachel **zuoberst** in **Zu erledigen**, nur am **Eventtag** (UTC), Link **`events.detail`** **`tab=billbro`**, Lucide **`receipt`** wie Event-Detail-Tab; **`RetroCleanupService.get_today_billbro_prompt_event`**. | Schneller Einstieg BillBro; gleiche Zugangslogik wie Tab (Organisator oder Zusage) |
...
**Paket 7f** (**§8.3.1**): erledigt — **`base.html`** = **done**; **übrige `templates/`** = **done**; **`dashboard/index.html`** bleibt **pending** bis PO-Freigabe (**§16.1**).
