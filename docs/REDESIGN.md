...
| BillBro-Phasenleiste     | `.billbro-workflow`, `.billbro-workflow__step`, `.billbro-workflow__step--done`, `.billbro-workflow__step--current`, `.billbro-workflow__index`                                                                                                                                                               | Event-Detail **BillBro**: horizontale Phasen **Schätzrunde → Rechnung → Gesamtbetrag → Abgeschlossen** (Zustand serverseitig aus `billbro_closed` / Rechnungs- / Final-Flags). **`components.css`**. |
| Bewertungsliste (Detail) | `.events-rating-others__entry` (+ Abstand zur nächsten Zeile per Adjacent-Sibling)                                                                                                                                                                                                                             | Tab **Bewertungen** auf Event-Detail: ein Eintrag pro fremder Bewertung, Trennlinie zwischen Blöcken. |


**CSS:** `static/css/v2/components.css` (Abschnitte „TOOL SURFACE“, „DISCLOSURE“, „CONTEXT ACTIONS“ [Legacy], „TOOL-STRIP“ / **`.tool-strip__actions`**, „SETTINGS NAV“, „EMPTY STATE“).
...
- **Tab Infos — erste Card:** Titel **„Event-Summary“**, Icon Lucide **`clipboard-list`** (wie GGL-Summary; Inhalt: Organisator, Teilnehmerzahl, Restaurant/Links/Notizen, RSVP-**`chip-select`** im **`card__header`**).
...
| Workflow (BillBro)                              | **`.billbro-workflow`** (Phasenleiste Event-Detail) + bestehende Cards/Formulare                                                                                                                  |
...
| Monatsessen-/Vereins-Kennzahlen mit Diagrammen   | Backend-Aggregation (hier: **`get_monatsessen_statistics`** in **`backend/services/monatsessen_stats.py`**) + **`metrics-spotlight`** + **`metrics-insight-panel`** + Chart.js (horizontale Balken, Pie); CSS **`.events-stats-chart`**; Referenz **`templates/events/index.html`** (`tab=stats`), JS **`static/js/v2/events-monatsessen-stats.js`**, Daten **`#events-monatsessen-charts-data`**. |
...
| 3 Events-Übersicht | erledigt | Wie zuvor; **Filter-Leiste:** nach **GET-Submit Filtern** einklappen + **`sessionStorage`** geschlossen (**§5.2.3**). Tool-Strip, Tabs **`tabs--panel`**, Tabellen. **Tab Statistiken:** Monatsessen-Fokus, Backend **`monatsessen_stats.py`**, Chart.js, Summary/Copy — Details **§11 (2026-04-05)** und **§12**. |
| 4 Event-Detail     | erledigt | **`detail.html`:** **`tabs--panel`**, Card **Event-Summary** (**`clipboard-list`**), Leiste **Bearbeiten** + **`sessionStorage`**, Lucide, **`data-table`** (Teilnehmer **`events-participants-table`**, Schätzungsrangliste), **`billbro-workflow`**, Bewertungen **`metrics-spotlight`** / **`empty-state`** — **§11**. |
...
- **2026-04-05:** **Event-Detail, Tab Infos:** Card bisher **„Event-Informationen“** umbenannt in **„Event-Summary“**; Card-Titel-Icon **Lucide `clipboard-list`** (Angleichung an GGL-Summary-Card). Template **`templates/events/detail.html`**.
...
- **2026-04-05:** **Phase 4 — `templates/events/detail.html`:** Breadcrumb durch **`.page-back`** ersetzt; **Lucide**-Makros wie Events-Index; Organisator-Leiste + **`sessionStorage`**; **`tabs tabs--panel`**; Infos: **Teilnehmer** und **Schätzungsrangliste** als **`data-table`**; BillBro: **`billbro-workflow`**; Bewertungen: **`metrics-spotlight`** / **`empty-state`** / **`events-rating-others__entry`**; doppelte **Rechnungsübersicht** entfernt; Skripte in **`{% block scripts %}`**. CSS: **`base.css`** (`.page-back`), **`components.css`**.
