...
- Ein **`details`**-Filter: **`id="ggl-filter-disclosure"`**; **Lucide** **`chevron-down`** / **`funnel`** am Primary **Filtern**; **`form-actions form-actions--start`** (Reihenfolge **Filtern** → **Zurücksetzen**), wie Events-Filter.
- **`{% block scripts %}`:** **`bindDisclosure('ggl-filter-disclosure', 'gourmen:gglFilterDisclosureOpen', false)`** — Standard **eingeklappt**; Tab-Wechsel (Dein Rang / Tabelle / Rennen) = Reload, Zustand bleibt über **`sessionStorage`**.
- Bereichs-Tabs: **`tabs tabs--panel`** (**§5.1**).
...
| 2 GGL-Pilot        | erledigt | `ggl/index.html`: **stat-tiles** (Tab Dein Rang), **Rang-Tabelle** statt Card-Liste; **Filter** + **Tabs** wie **§5.2.3** / **§5.1** (**`tabs--panel`**, **`funnel`**, **`form-actions--start`**, **`ggl-filter-disclosure`** + **`gourmen:gglFilterDisclosureOpen`**); Lucide-Sprite per `url_for`; CSS in `components.css`; Tag `**pre-phase-2`** gesetzt                                                                                                                                                                                                                    |
| 3 Events-Übersicht | erledigt | `events/index.html` + `backend/routes/events.py`: **Planung + Filter** als **Tool-Strip** (**§5.2.3**): je `details.disclosure` in **`card.card--filter.tool-surface`**; **sessionStorage**-Zustand (Standard **eingeklappt**); **Filter** **über** Tabs, global; Tabs **Kommend / Archiv / Statistiken**; `**.data-table-wrap` / `.data-table`**; kein Tab „Übersicht“; **rating_prompt_event**-Alert; Kommend-Leerzustand `**empty-state`** / `**empty-state--filtered**`; Spalte **Typ** = Icon; Archiv **ohne** Küche; **stat-tiles**; Lucide-Sprite; optional **`.tabs--panel`** (**§5.1**) |
