...
| 4b | done (Code; Merge + Prod-Migration/Seed offen) | `phase/04b-workspace-bank-import` | 2026-07-13 | **Refokus Buchhaltung** komplett: ZKB-CSV-Import (Upload, Dedup, Sammelbuchungs-Splitting, Auto-Vorschläge, Review-Screen), Offene Posten (`MemberClaim` inkl. Teilzahlungen und Aufrundungs-Verbuchung, Tab + Dashboard-Integration), BillBro-Zahlweg (Vereinskonto vs. auslegendes Mitglied inkl. Eingangs-Bestätigung), Kontenplan verschlankt (18 Konten, Seed mit Alt-Konten-Cleanup), EventType `ESSEN_BUCHHALTUNG`, Budget-Vorschlag bei Jahreseröffnung (Beiträge = aktive Mitglieder × `membership_fee_rappen`, Rest = Vorjahres-Ist). Migration `a4b7c2d9e015`, PWA 3.15.0. Spec: `docs/capabilities/accounting.md` Sektion 6 + 11. Nach Merge: Migration automatisch via `preDeployCommand`; Seed-Script manueller Einmal-Schritt. |
