...
- **Merch v2** (`MERCH_V2_ENABLED`): gemeinsame Decorators und Flag-Helfer in `backend/routes/merch_access.py` (`marketing_chief_or_admin_required`, `treasury_marketing_or_admin_required`, `merch_legacy_receivables_required`, `require_merch_v2_enabled`). Cockpit-Blueprint `backend/routes/merch_admin.py` unter Praefix `/admin/merch-v2`. Legacy-Receivables nur lesend: `admin` `/admin/merch/legacy-receivables` (Marketingchef, Schatzmeister, Admin). Lifecycle `MerchRoundService.apply_transition` inkl. Order-Hooks (Lock verwirft DRAFT-Orders, Re-Open setzt CONFIRMED zurueck, ORDERED facturiert, Cancel storniert aktive Orders; PAID bleibt PAID). `MerchOrderService` ergaenzt Storno durch Mitglied, Abhol-/Bezahl-Markierung, Auto-Close Runde. Mitgliedsshop unter Blueprint `merch`. Spec `docs/capabilities/merch.md`.
