...
- **Merch v2** (`MERCH_V2_ENABLED`): gemeinsame Decorators und Flag-Helfer in `backend/routes/merch_access.py` (`marketing_chief_or_admin_required`, `treasury_marketing_or_admin_required`, `require_merch_v2_enabled`). Cockpit-Blueprint `backend/routes/merch_admin.py` unter Praefix `/admin/merch-v2` (Runden anlegen/Oeffnen, Round-Detail, Sortiment; klassischer Merch-Admin bleibt unter `admin` `/admin/merch`). Lifecycle-Helfer `MerchRoundService`: `create_draft_round`, `apply_transition` (OPEN nur mit mind. einer `MerchRoundItem`), `add_variant_to_round`, `remove_round_item` (Sortiment nur bei DRAFT). `MerchOrderService`: `get_or_create_draft_order`, `set_cart_line`, `recalculate_draft_totals`, `confirm_order` (Mitgliedsshop unter Blueprint `merch`). Spec `docs/capabilities/merch.md`.
