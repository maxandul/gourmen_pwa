...
- **Merch v2** (`MERCH_V2_ENABLED`): gemeinsame Decorators und Flag-Helfer in `backend/routes/merch_access.py` (`marketing_chief_or_admin_required`, `merch_statistics_view_required` fuer Vereins-/Jahresstatistik Lesen durch Marketingchef oder Schatzmeister, `treasury_marketing_or_admin_required`, `merch_legacy_receivables_required`, `require_merch_v2_enabled`). Cockpit `backend/routes/merch_admin.py` unter `/admin/merch-v2`; Stammdaten **Lieferanten** (`GET/POST /suppliers`, Archivierung); Jahresreport `GET /statistics`, CSV `GET /statistics/export.csv`; Lieferantenbeleg nach Drive ueber optional `MERCH_SUPPLIER_INVOICE_DRIVE_FOLDER_ID`. Legacy-Receivables nur lesend: `admin` `/admin/merch/legacy-receivables`. Mitglieds-Merch-Historie (v2): Admin-Mitglied-Detail. Lifecycle `MerchRoundService.apply_transition` inkl. Order-Hooks. `MerchOrderService`: Warenkorb, Storno, Abhol-/Bezahl-Markierung. `MerchStatisticsService`: Runden- und Saison-KPIs. Shop unter Blueprint `merch`. Spec `docs/capabilities/merch.md`.
