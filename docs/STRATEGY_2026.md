...
| Merch-Produktbilder | **Google Shared Drive** (`Verein/Merch/Bilder/`), Auslieferung via Backend-Proxy `/merch/image/<article_id>` mit Cache | Marketingchef pflegt Bilder autonom, kein Repo-Commit pro Bild. Konsistent mit Drive-Strategie für Belege. Aktualisiert 2026-05-15 mit Merch-Capability (vorher: Repo unter `static/img/merch/`). |
...
- **Disclosure/Aktionen-Pattern neu denken** — aktuell sind Aktionen wie Suchen, Bearbeiten und ähnliche klappbar in Sections dargestellt, teilweise inkonsistent über Seiten hinweg. Soll von Grund auf nach Best Practice neu konzipiert werden, inkl. **Seite-Teilen-Funktion** (Link kopieren / Native Share-API). Eigene kleine UX-Konzept-Phase — Anker auf `docs/UI.md`. Aufgenommen 2026-05-15.
- **PWA-Domain-Migration** — Mitglieder haben die PWA noch unter der Railway-URL installiert, mittlerweile existiert eine Custom-Domain. PWA-Installationen sind an die Origin gebunden — ein simpler Redirect bewegt installierte Versionen nicht mit. Braucht eigenes Konzept: Strategie zum Umzug (Banner mit Re-Install-Hinweis? Service-Worker-Migration? Manifest-Update?), Risiken (Push-Subscriptions an alte Origin gebunden), Kommunikation an Mitglieder. Eigene mittlere Capability. Aufgenommen 2026-05-15.
- **Performance + Offline-Verhalten** — App lädt teilweise langsam (volle Page-Reloads statt App-Shell-Pattern), kein Hinweis bei langsamer/fehlender Internetverbindung. Best Practice für PWAs: Service-Worker-Caching (Stale-While-Revalidate für Listen, Cache-First für statische Assets), App-Shell-Pattern für Navigation, `navigator.onLine`-Indikator, Offline-Hinweis-Banner, ggf. lesbare Offline-Sicht für zwischengespeicherte Daten. Eigene mittlere Capability. Aufgenommen 2026-05-15.
- **Link-Underline entfernen** — in der App existieren noch Links, deren Schrift beim Klick/Aktiv unterstrichen wird. Soll konsistent entfernt werden (Design-Token oder Reset in `static/css/v2/base.css`). Kleiner Cursor-Auftrag, kann mit Disclosure-Redesign oder UX-Polish-Sammel-Phase mitlaufen. Aufgenommen 2026-05-15.
...
| **2026-05-15** | **Merch-Capability komplett konzipiert (Phase 10), Bilder via Drive statt Repo** | MVP-Punkt 4 abgeschlossen als Konzept. Vollständiges Prozess-Redesign mit Bestellrunden als zentralem Konzept (Lifecycle Round + Order, Pricing dreistufig: Listenpreis/Effektivpreis/Mitgliederpreis, GV-Subvention pro Runde, Forderungs-Stub mit definierter Buchhaltungs-Schnittstelle). BillBro NICHT verzahnt mit Merch (BillBro ist event-spezifisch). Bilder-Strategie aktualisiert: Drive mit Backend-Proxy statt Repo-Pfad — Marketingchef arbeitet autonom, kein Repo-Commit pro Bild. Spec autoritativ in `docs/capabilities/merch.md`, Phase-Briefing in `docs/initiatives/workspace-railway/PHASE_10_MERCH.md`. |
