...
@migrate_bp.route('/admin/migrate/merch', methods=['POST'])
@login_required
def migrate_merch():
    """Temporärer Endpoint zum Erstellen der Merch-Tabellen"""
    # Nur Admins können Migrationen ausführen
    if not current_user.is_admin:
        return jsonify({'error': 'Nur Admins können Migrationen ausführen'}), 403
